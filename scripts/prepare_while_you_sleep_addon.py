...
<style id="while-you-sleep-nav-style">
  ...
  .dbr-genie-bolt {
    ...
    flex: 0 0 auto !important;
    color: #eba941 !important;
    filter: drop-shadow(0 0 4px rgba(235,169,65,.35)) !important;
    transition: transform 220ms cubic-bezier(.23,1,.32,1), filter 220ms cubic-bezier(.23,1,.32,1) !important;
    transform-origin: center !important;
  }
  .dbr-genie-hero-link:hover {
    transform: scale(1.04) !important;
    background: oklch(0.62 0.18 280 / 0.18) !important;
    box-shadow: 0 0 24px oklch(0.78 0.15 175 / 0.28) !important;
  }
  .dbr-genie-hero-link:hover .dbr-genie-bolt {
    transform: translate(3px,-3px) rotate(12deg) scale(1.14) !important;
    filter: drop-shadow(0 0 7px rgba(235,169,65,.62)) !important;
  }
  .dbr-genie-hero-link:active { transform: scale(.97) !important; }
  .dbr-genie-hero-link:active .dbr-genie-bolt {
    transform: translate(1px,-1px) rotate(6deg) scale(1.06) !important;
  }
  @media (prefers-reduced-motion: reduce) {
    .dbr-genie-hero-link,
    .dbr-genie-bolt { transition: none !important; }
  }
...
(() => {
  const addLinks = () => {
    ...
    zoomLinks.forEach((zoom) => {
      ...
      const link = document.createElement('a');
      link.href = '/genies';
      link.innerHTML = 'AI Genie Team<svg class="dbr-genie-bolt" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13.2 1.8 4.8 13h5.7l-1.4 9.2L19.2 10h-6.1l.1-8.2Z"/></svg>';
      link.className = `${zoom.className} dbr-genie-hero-link`;
      link.setAttribute('aria-label', 'Open AI Genie Team');
